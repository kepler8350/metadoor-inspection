...
# 메타도어 설치 리스트 - 기본값 (사용자가 PDF 내용을 제공하면 업데이트)
METADOOR_DATA = {
    '금정구': [
        {'name': '금정구청', 'address': '부산시 금정구'},
        {'name': '금정도서관', 'address': '부산시 금정구'},
        {'name': '금정보건소', 'address': '부산시 금정구'}
    ],
    '기장군': [
        {'name': '기장군청', 'address': '부산시 기장군'},
        {'name': '기장도서관', 'address': '부산시 기장군'},
        {'name': '정관중학교', 'address': '부산시 기장군'}
    ],
    '남구': [
        {'name': '남구청', 'address': '부산시 남구'},
        {'name': '용호문화센터', 'address': '부산시 남구'},
        {'name': '남부경찰서', 'address': '부산시 남구'}
    ],
    '동구': [
        {'name': '동구청', 'address': '부산시 동구'},
        {'name': '초량문화센터', 'address': '부산시 동구'},
        {'name': '동부경찰서', 'address': '부산시 동구'}
    ],
    '동래구': [
        {'name': '동래구청', 'address': '부산시 동래구'},
        {'name': '동래도서관', 'address': '부산시 동래구'},
        {'name': '온천장센터', 'address': '부산시 동래구'}
    ],
    '부산진구': [
        {'name': '부산진구청', 'address': '부산시 부산진구'},
        {'name': '부산진도서관', 'address': '부산시 부산진구'},
        {'name': '서면문화센터', 'address': '부산시 부산진구'}
    ],
    '북구': [
        {'name': '북구청', 'address': '부산시 북구'},
        {'name': '북부도서관', 'address': '부산시 북구'},
        {'name': '구포센터', 'address': '부산시 북구'}
    ],
    '사상구': [
        {'name': '사상구청', 'address': '부산시 사상구'},
        {'name': '사상도서관', 'address': '부산시 사상구'},
        {'name': '삼락공원', 'address': '부산시 사상구'}
    ],
    '사하구': [
        {'name': '사하구청', 'address': '부산시 사하구'},
        {'name': '감천문화마을', 'address': '부산시 사하구'},
        {'name': '사하도서관', 'address': '부산시 사하구'}
    ],
    '서구': [
        {'name': '서구청', 'address': '부산시 서구'},
        {'name': '암남공원', 'address': '부산시 서구'},
        {'name': '서부센터', 'address': '부산시 서구'}
    ],
    '수영구': [
        {'name': '수영구청', 'address': '부산시 수영구'},
        {'name': '수영도서관', 'address': '부산시 수영구'},
        {'name': '광안리관리소', 'address': '부산시 수영구'}
    ],
    '연제구': [
        {'name': '연제구청', 'address': '부산시 연제구'},
        {'name': '연제도서관', 'address': '부산시 연제구'},
        {'name': '연제센터', 'address': '부산시 연제구'}
    ],
    '영도구': [
        {'name': '영도구청', 'address': '부산시 영도구'},
        {'name': '영도도서관', 'address': '부산시 영도구'},
        {'name': '절영로역사관', 'address': '부산시 영도구'}
    ],
    '중구': [
        {'name': '중구청', 'address': '부산시 중구'},
        {'name': '중앙도서관', 'address': '부산시 중구'},
        {'name': '항만공사', 'address': '부산시 중구'}
    ],
    '해운대구': [
        {'name': '해운대구청', 'address': '부산시 해운대구'},
        {'name': '해운대도서관', 'address': '부산시 해운대구'},
        {'name': '해수욕장관리소', 'address': '부산시 해운대구'}
    ]
}
    ...
    <div class="app-content">
      ...
      <div class="screen district-screen">
        ...
        <div class="content-area">
          <div class="left-panel" id="district-list"></div>
          <div class="right-panel" id="location-lists"></div>
        </div>
      </div>
      
      <div class="screen inspection-screen">
        <div class="inspection-info">
          <div class="inspection-name" id="insp-name">사상구 - 사상도서관</div>
          <div style="font-size: 11px; color: rgba(255, 255, 255, 0.7);">점검 입력 / 3단계</div>
        </div>
        <div class="form-group">
          <div class="form-title">점검 항목 (11개)</div>
          <select id="insp-type">
  ...
  <script>
    ...
    function initApp() {
      ...
      Object.keys(METADOOR_DATA).forEach(district => {
        ...
        METADOOR_DATA[district].forEach(location => {
    ...
    function handleSave() { alert('임시 저장되었습니다.'); }
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .table th { padding: 12px; text-align: left; font-weight: 600; color: #333; border-bottom: 1px solid #eee; background: #f9f9f9; }
      ...
      fetch('/api/inspections')
        ...
        .then(data => {
          ...
          data.forEach(insp => {
            ...
            row.innerHTML = `<td>\${insp.district}</td><td>\${insp.location}</td><td>\${insp.item}</td><td><span class="\${statusClass}">\${insp.status}</span></td><td>\${insp.content || '-'}</td><td>\${date}</td>`;
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; background: linear-gradient(135deg, #1e5a96 0%, #164a7a 100%); min-height: 100vh; display: flex; justify-content: center; align-items: center; padding: 20px; }
    .login-card { background: white; border-radius: 16px; padding: 40px; box-shadow: 0 10px 40px rgba(0,0,0,0.2); width: 100%; max-width: 400px; text-align: center; }
    .logo { font-size: 56px; margin-bottom: 16px; }
