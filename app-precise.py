...
LOCATIONS = {
    ...
    '사상구': ['사상구청 메타도어', '사상도서관 메타도어', '삼락공원 메타도어'],
...
body{font-family:'Segoe UI','Apple Color Emoji',sans-serif;display:flex;justify-content:center;align-items:center;min-height:100vh;background:#eee;padding:10px}
...
.card{background:white;border-radius:16px;padding:32px;text-align:center;width:100%}
...
.btn{width:100%;padding:11px;background:#1e5a96;color:white;border:none;border-radius:8px;font-weight:600;cursor:pointer;font-size:14px}
...
.dist-btn{width:100%;padding:10px 12px;background:white;border:1px solid #ddd;border-radius:6px;font-size:12px;margin-bottom:4px;cursor:pointer;text-align:left;font-weight:500}
.dist-btn.active{background:#1e5a96;color:white;border-color:#1e5a96}
.loc-item{padding:10px 12px;background:white;border:1px solid #eee;border-radius:6px;margin-bottom:8px;cursor:pointer;font-size:12px;color:#333}
...
</style></head><body><div class="phone"><div class="notch">9:41</div><div class="status"><span>📶</span><span>🔋</span></div><div class="screen login-scr active"><div class="card"><div class="logo">🏛️</div><div class="title">메타도어</div><div class="subtitle">유지보수 점검 시스템</div><input type="text" id="user" value="user" class="input-field" placeholder="아이디"><input type="password" id="pass" value="user123" class="input-field" placeholder="비밀번호"><button class="btn" onclick="login()">로그인</button><div style="font-size:10px;color:#999;margin-top:12px">권한자가 등록한 직원만<br>로그인 가능합니다</div></div></div><div class="screen list-scr"><div class="header">구 선택</div><div style="display:flex;flex:1"><div class="left" id="dlist"></div><div class="right" id="rlist"><div class="help-text">왼쪽에서 구를 선택하세요</div></div></div></div><div class="screen list-scr"><div class="header">설치 위치 선택</div><div style="display:flex;flex:1"><div class="left" id="dlist2"></div><div class="right" id="llist"></div></div></div><div class="screen form-scr"><div class="form-header"><div class="form-title" id="ftitle"></div><div class="form-step">점검 입력 / 3단계</div></div><div class="form-group"><label class="label">점검 항목</label><select id="item"></select></div><div class="form-group"><label class="label">조사 내용</label><textarea id="content" placeholder="이상 발견 시 조사 내용 기록"></textarea></div><div class="form-group"><label class="label">점검 상태</label><select id="status"><option value="정상">✓ 정상</option><option value="이상">✕ 이상</option></select></div><div class="btn-group"><button class="btn-2 btn-secondary" onclick="show(1)">이전</button><button class="btn-2 btn-primary" onclick="submit()">완료</button></div></div></div></div><script>
...
function initDist(){const d=document.getElementById('dlist');Object.keys(LOCATIONS).forEach(k=>{const b=document.createElement('button');b.className='dist-btn';b.textContent=k;b.onclick=()=>selectDist(k,b);d.appendChild(b)})}
...
function selectLoc(l){loc=l;document.getElementById('ftitle').textContent=dist+' - '+l;initItem();initDist2();show(3)}
function initDist2(){const d=document.getElementById('dlist2');d.innerHTML='';Object.keys(LOCATIONS).forEach(k=>{const b=document.createElement('button');b.className='dist-btn';b.textContent=k;if(k===dist)b.classList.add('active');d.appendChild(b)})}
...
function submit(){const item=document.getElementById('item').value;if(!item||item==='-- 선택 --'){alert('점검 항목을 선택하세요');return}fetch('/api/submit',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({dist,loc,item,content:document.getElementById('content').value,status:document.getElementById('status').value})}).then(()=>{alert('점검이 완료되었습니다!');show(1);initDist()});}
</script></body></html>'''
